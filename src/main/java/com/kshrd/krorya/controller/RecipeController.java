...
import com.kshrd.krorya.model.enumeration.CusineEnum;
...
//                                                                  @Pattern(regexp = "^[a-zA-Z\\s]+$", message ="invalid input. Only text characters are allowed")
    ...
    @Operation(summary = "filter recipe by cooking level ", description = "retreive recipe by cooking level ")
    @GetMapping("/recipes/CookingLevel")
    public ResponseEntity<ApiResponse<List<RecipeDTO>>> filterRecipeByCookingLevel(
            @RequestParam CookingLevelEnum cookingLevelEnum) {
        List<RecipeDTO> recipeDTOS = recipeService.findRecipeByCookingLevel(cookingLevelEnum);
        ApiResponse<List<RecipeDTO>> apiResponse = ApiResponse.<List<RecipeDTO>>builder()
                .localDateTime(LocalDateTime.now())
                .message("recipe has filter by cooking level successfully")
                .payload(recipeDTOS)
                .code(201)
                .status(HttpStatus.OK)
                .build();
    ...
    @Operation(summary = "filter recipe by cuisine", description = "retreive recipe by cuisine")
    @GetMapping("/recipes/cuisine")
    public ResponseEntity<ApiResponse<List<RecipeDTO>>> filterRecipeByCuisines(
            @RequestParam CusineEnum cusineEnum
    ) {
        List<RecipeDTO> recipeDTOS = recipeService.findRecipeByCuisine(cusineEnum);
        ApiResponse<List<RecipeDTO>> apiResponse = ApiResponse.<List<RecipeDTO>>builder()
                .localDateTime(LocalDateTime.now())
    ...
    @Operation(summary = "filter recipe by cooking_times", description = "retreive recipe by cooking_times")
    @GetMapping("/recipes/cookingTime")
    public ResponseEntity<ApiResponse<List<RecipeDTO>>> filterRecipeByCookingTime(
            @RequestParam CookingTimeEnum cookingTimeEnum
    ) {
        List<RecipeDTO> recipeDTOS = recipeService.findRecipeByCookingTime(cookingTimeEnum);
        ApiResponse<List<RecipeDTO>> apiResponse = ApiResponse.<List<RecipeDTO>>builder()
                .localDateTime(LocalDateTime.now())
                .message("Recipe has been find successfully")
                .payload(recipeDTOS)
    ...
    @Operation(summary = "filter recipe by number of ingredients", description = "retreive recipe by number of ingredients")
    @GetMapping("/recipes/ingredients")
    public ResponseEntity<ApiResponse<List<RecipeDTO>>> filterRecipeByIngredients(
            @RequestParam IngredientsEnum ingredientsEnum
    ) {
        List<RecipeDTO> recipeDTOS = recipeService.findRecipeByNumberOfIngredients(ingredientsEnum);
        ApiResponse<List<RecipeDTO>> apiResponse = ApiResponse.<List<RecipeDTO>>builder()
                .localDateTime(LocalDateTime.now())
                .message("Recipe has been find successfully")
                .payload(recipeDTOS)
